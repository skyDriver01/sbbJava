...
public class aufg1 {
    public static void main(String[] args) {
        ...
        System.out.println(getTheNumbersBetweenTenAndZeroWithoutTheFive(numbers, 0, 10, 5));
    }

    public static int getTheNumbersBetweenTenAndZeroWithoutTheFive(int[] numberArray, int min, int max, int notWanted) {
        int counter = 0;
        for (int i = 0; i < numberArray.length; i++) { //
            if (numberArray[i] <= max && numberArray[i] >= min && numberArray[i] != notWanted) {
                counter++;
            }
        }
        return counter;
