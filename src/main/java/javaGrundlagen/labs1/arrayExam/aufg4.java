...
public class aufg4 {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(makeAllNumbersPositiveInTheArray(numbers)));
    }
    public static int[] makeAllNumbersPositiveInTheArray(int[] getTheArrayAbove){
        int[] makeThemPositive = new int[getTheArrayAbove.length];
        int arrayIndex = 0;
        for (int i = 0; i < getTheArrayAbove.length; i++) {
            if (getTheArrayAbove[i] < 0) {
                makeThemPositive[arrayIndex] = getTheArrayAbove[i] * -1;
                arrayIndex++;
            }
        }
        return Arrays.copyOfRange(makeThemPositive, 0 , arrayIndex);
    }
}